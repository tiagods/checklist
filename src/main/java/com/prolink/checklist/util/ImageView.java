...
public class ImageView extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public ImageView(JFrame frame, Cliente cliente,Map<Path,Mensageria> arquivos) {
		super(frame,true);
		initComponents();
	...
	public void	initComponents(){
		...
		btnValidar.addActionListener(controller);
		{
			...
			txtpnOChecklistConsegue.setText(
					"O Checklist consegue ler arquivos PDF gravados no formato TEXTO, "
					+ "um formato que permite leitura de caracteres (chamados de STRINGS), "
					+ "em 1º caso pode ser impossivel a leitura de texto, "
					+ "ai entra o recurso de OCR (Inteligencia Artificial) que lê imagens, "
					+ "essa tecnologia permite extrair textos de imagens, "
					+ "esse resultado é retornado nessa area para validação manual que trata PDF do tipo Imagem."
					+ "\r\nO PDF que teve o seu nome validado mas o conteudo não foi lido, será mostrado no painel a direita, "
					+ "no momento válido apenas para a 1ª página do arquivo PDF, "
					+ "Todos os resultados, VALIDOS ou INVALIDOS são retornados para sua ultima análise se achar necessário, "
					+ "você deve comparar os campos que achar conveniente e Validar se concordar com o resultado mostrado, "
					+ "caso contrário Rejeitar. "
					+ "Sua ação é encaminhada imediatamente para a tabela de resultados no menu principal."
					);
